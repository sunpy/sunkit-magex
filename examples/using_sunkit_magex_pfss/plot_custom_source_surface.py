...
header_dipole = sunkit_magex.pfss.utils.carr_cea_wcs_header(Time('2020-1-1'), br_dipole.shape)
map_dipole = sunpy.map.Map((br_dipole.T, header_dipole))

# Closed outer boundary:
br_zeros = np.zeros((nphi, ns))
header_zeros = sunkit_magex.pfss.utils.carr_cea_wcs_header(Time('2020-1-1'), br_zeros.shape)
...
pfss_in_radial = sunkit_magex.pfss.Input(map_dipole, nrho, rss)  # 'radial'
pfss_in_closed = sunkit_magex.pfss.Input(map_dipole, nrho, rss, map_zeros)

###############################################################################
# Now calculate the PFSS solutions.
pfss_out_radial = sunkit_magex.pfss.pfss(pfss_in_radial)
pfss_out_closed = sunkit_magex.pfss.pfss(pfss_in_closed)
...
tracer = sunkit_magex.pfss.tracing.PerformanceTracer()
